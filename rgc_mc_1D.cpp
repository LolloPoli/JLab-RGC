...
// rsync -avz -e "ssh -J lpolizzi@login.jlab.org" lpolizzi@ifarm:/lustre24/expphy/volatile/clas12/lpolizzi/sidis/rgc/mc_output/sum22_neg/ /Users/lorenzopolizzi/Desktop/PhD/JLAB/rgc/MC_summer22_neg
// 
                          ...
                          const char* campaign = "fall22")
{
    ...
    double pBeam = 0.90; // typical value for CLAS12
...
void rgc_mc_1D(const char* period, const char* target) {
    ...
    //string inputDir = "fall22_NH3_data";
    // string inputDir = string(period) + "_" + string(target) + "_data"; // TO ADD WHEN WE WILL HAVE OTHER PERIOD
    vector<string> rootFiles = {"MC_summer22_neg", "MC_summer22_pos"};
    ...
    */
    ...
    // creo un output root 
    TString outputFile = Form("plot_mc_%s_%s_kaonp.root", target, period);
    double torus = -1;
    TString csv_filename = Form("output_RGC_mc_%s_%s.csv", target, period);
    //const char* outputFile = "plot_mc_sum22_kaonp.root"; 
    //double torus = -1;
    //const char* csv_filename = "output_RGC_mc_sum22.csv";
    ...
    mg_Aut2D_xB_2->SetTitle("A_{UL}^{sin#Phi_{h}} vs x_{B} (x_{B}-Q^{2} bin) | lepton frame; x_{B}; A_{UL}^{sin(#Phi_{h})}");
    ...
    mg_Aut2D_xB_2->GetYaxis()->SetRangeUser(-0.2, 0.2);
    ...
    mg_Aut2D_n_xB_2->GetYaxis()->SetRangeUser(-0.2, 0.2);
    ...
    mg_Aut2D_DUL_xB_2->GetYaxis()->SetRangeUser(-0.2, 0.2);
    guideLine11->Draw();
    legend->Draw();
    ...
    for (size_t i = 0; i < graphs_AUL_vs_z.size(); ++i) {
        ...
        graphs_AUL_vs_z[i]->SetTitle(Form("A_{UL}^{sin#Phi_{h}} vs z | %s | lepton frame; z; A_{UL}^{sin(#Phi_{h})}",titles_AUL_vs_z[i].c_str()));
        graphs_AUL_vs_z[i]->Draw("AP");
        graphs_AUL_vs_z[i]->GetYaxis()->SetRangeUser(-0.2, 0.2);
    ...
    for (size_t i = 0; i < graphs_DUL_vs_z.size(); ++i) {
        ...
        graphs_DUL_vs_z[i]->GetYaxis()->SetRangeUser(-0.2, 0.2);
    ...
    mg_Aut2D2_xB_2->SetTitle("A_{UL}^{sin2#Phi_{h}} vs x_{B} (x_{B}-Q^{2} bin) | lepton frame; x_{B}; A_{UL}^{sin(2#Phi_{h})}"); // #sqrt{2#epsilon(1+#epsilon)}
    ...
    mg_Aut2D2_xB_2->GetYaxis()->SetRangeUser(-0.2, 0.2);
    guideLine11->Draw();
    ...
    mg_Aut2D2_DUL_xB_2->GetYaxis()->SetRangeUser(-0.2, 0.2);
    guideLine11->Draw();
    legend2->Draw();
    ...
    for (size_t i = 0; i < graphs_AUL2_vs_z.size(); ++i) {
        ...
        TCanvas* c = new TCanvas(cname, ctitle, 800, 600);
        graphs_AUL2_vs_z[i]->SetTitle(Form("A_{UL}^{sin2#Phi_{h}} vs z | %s | lepton frame; z; A_{UL}^{sin(2#Phi_{h})}",titles_AUL2_vs_z[i].c_str()));
        graphs_AUL2_vs_z[i]->Draw("AP");
        graphs_AUL2_vs_z[i]->GetYaxis()->SetRangeUser(-0.2, 0.2);
    ...
    for (size_t i = 0; i < graphs_DAUL2_vs_z.size(); ++i) {
        ...
        graphs_DAUL2_vs_z[i]->GetYaxis()->SetRangeUser(-0.2, 0.2);
    ...
    mg_All2D_xB_0->SetTitle("A_{LL} vs x_{B} (x_{B}-Q^{2} bin) | lepton frame; x_{B}; A_{LL}");
    ...
    mg_All2D_xB_0->GetYaxis()->SetRangeUser(-0.1, 0.3);
    ...
    mg_DAll2D_xB_0->GetYaxis()->SetRangeUser(-0.1, 0.3);
    ...
    for (size_t i = 0; i < graphs_ALL0_vs_z.size(); ++i) {
        ...
        TCanvas* c = new TCanvas(cname, ctitle, 800, 600);
        graphs_ALL0_vs_z[i]->SetTitle(Form("A_{LL} vs z | %s | lepton frame; z; A_{LL}",titles_ALL0_vs_z[i].c_str()));
        graphs_ALL0_vs_z[i]->Draw("AP");
        graphs_ALL0_vs_z[i]->GetYaxis()->SetRangeUser(-0.1, 0.3);
    ...
    for (size_t i = 0; i < graphs_DALL0_vs_z.size(); ++i) {
        ...
        graphs_DALL0_vs_z[i]->GetYaxis()->SetRangeUser(-0.1, 0.3);
    ...
    mg_All2D_xB_cos->SetTitle("A_{LL}^{cos#Phi_{h}} vs x_{B} (x_{B}-Q^{2} bin) | lepton frame; x_{B}; A_{LL}^{cos#Phi_{h}}");
    ...
    mg_All2D_xB_cos->GetYaxis()->SetRangeUser(-0.2, 0.2);
    ...
    mg_DAll2D_xB_cos->GetYaxis()->SetRangeUser(-0.2, 0.2);
    ...
    for (size_t i = 0; i < graphs_ALL_cos_vs_z.size(); ++i) {
        ...
        TCanvas* c = new TCanvas(cname, ctitle, 800, 600);
        graphs_ALL_cos_vs_z[i]->SetTitle(Form("A_{LL}^{cos#Phi_{h}} vs z | %s | lepton frame; z; A_{LL}^{cos#Phi_{h}}",titles_ALL_cos_vs_z[i].c_str()));
        graphs_ALL_cos_vs_z[i]->Draw("AP");
        graphs_ALL_cos_vs_z[i]->GetYaxis()->SetRangeUser(-0.2, 0.2);
    ...
    for (size_t i = 0; i < graphs_DALL_cos_vs_z.size(); ++i) {
        ...
        graphs_DALL_cos_vs_z[i]->GetYaxis()->SetRangeUser(-0.2, 0.2);
    ...
    mg_Alu2D_xB_sin->SetTitle("A_{LU}^{sin#Phi_{h}} vs x_{B} (x_{B}-Q^{2} bin) | lepton frame; x_{B}; A_{LU}^{sin#Phi_{h}}");
    ...
    for (size_t i = 0; i < graphs_ALU_sin_vs_z.size(); ++i) {
        ...
        TCanvas* c = new TCanvas(cname, ctitle, 800, 600);
        graphs_ALU_sin_vs_z[i]->SetTitle(Form("A_{LU}^{sin#Phi_{h}} vs z | %s | lepton frame; z; A_{LU}^{sin#Phi_{h}}",titles_ALU_sin_vs_z[i].c_str()));
