...
// rsync -avz -e "ssh -J lpolizzi@login.jlab.org" lpolizzi@ifarm:/lustre24/expphy/volatile/clas12/lpolizzi/sidis/rgc/fall22/NH3/pion/ /Users/lorenzopolizzi/Desktop/PhD/JLAB/rgc/fall22_matt
                          ...
                          bool use_spinstate_for_helicity = true)
{
    ...
    const double dilution = 0.22; // 0.24 for NH3 and 0.32 for ND3
...
int getBinIndex_xB(double xB){
    vector<double> xB_edges = {0.0, 0.1362, 0.1615, 0.1839, 0.2059, 0.2332, 0.2703, 0.3273, 0.45, 1.0};
    //vector<double> xB_edges = {0.0, 0.1522, 0.1842, 0.2122, 0.2426, 0.2763, 0.3208, 0.3883, 1.0};
    for(size_t i = 0; i < xB_edges.size() - 1; i++){
        if(xB >= xB_edges[i] && xB < xB_edges[i+1]){
            return i + 1;  // bin numerati da 1 a 9
        }
    }
    // includiamo il bordo superiore nell’ultimo bin
    if(xB == 1.0) return 9;
...
void rgc_cc_mat() {
    ...
    string inputDir = "fall22_matt";
    //vector<string> rootFiles = {"fall22_NH3_data_new", "sum22_NH3_data"};

    TChain chainKaonP("Pion+");
    int fileCount = 0;
    
    for (const auto &entry : fs::directory_iterator(inputDir)) {
        if (entry.path().extension() == ".root") {
            string filePath = entry.path().string();
            chainKaonP.Add(Form("%s/Pion+", filePath.c_str()));
            fileCount++;
        }
    }
    /*
    ...
    */
    ...
    // creo un output root 
    const char* outputFile = "plot_cc_Mat.root"; 
    double torus = -1;
    ...
    chainKaonP.SetBranchAddress("pion_px", &kaonp_px);
    chainKaonP.SetBranchAddress("pion_py", &kaonp_py);
    chainKaonP.SetBranchAddress("pion_pz", &kaonp_pz);
    chainKaonP.SetBranchAddress("pion_mom", &kaonp_Ph);
    ...
    chainKaonP.SetBranchAddress("pion_Pt", &kaonp_PhT);
    chainKaonP.SetBranchAddress("Pt_over_zQ", &kaonp_Pt_zQ);
    chainKaonP.SetBranchAddress("pion_phi_lab", &kaonp_Phi);
    chainKaonP.SetBranchAddress("pion_theta", &kaonp_Theta);
    chainKaonP.SetBranchAddress("eta", &kaonp_eta);
    chainKaonP.SetBranchAddress("pion_phi_h", &kaonp_Phi_h);
    ...
    TH1D target_abs_pol ("_target_abs_polarization", "NH_{3} polarization | 1.2 < Mom < 8 GeV ; polarization (z axis); count", 100, 0.0, 1);
    ...
    vector<vector<double>> vec_kaonp_phih_1d(binxB);
    vector<vector<double>> vec_helicity_1d(binxB);
    vector<vector<double>> vec_kaonp_2phih_1d(binxB);
    vector<vector<double>> vec_kaonp_z_1d(binxB);
    vector<vector<double>> vec_kaonp_Pt_1d(binxB);
    vector<vector<double>> vec_kaonp_xB_1d(binxB);
    vector<vector<double>> vec_kaonp_Q2_1d(binxB);
    vector<vector<double>> vec_kaonp_y_1d(binxB);
    vector<vector<double>> vec_kaonp_pol_1d(binxB);
    vector<vector<double>> vec_kaonp_eps_1d(binxB);
    vector<vector<double>> vec_kaonp_spin_1d(binxB);
    vector<vector<double>> vec_kaonp_sintheta_1d(binxB);
    vector<vector<double>> vec_kaonp_phih_plus_1d(binxB);
    vector<vector<double>> vec_kaonp_phih_minus_1d(binxB);
    // xQ2 bins
    ...
    for (Long64_t i = 0; i < nEntries_kp; i++) {
        ...
        if (kaonp_y <= 0.2 || kaonp_y > 0.8) continue;
        ...
        if(std::abs(kaonp_chi2pid) < 3){
            ...
            if(kaonp_track_pid == 211){
                ...
                double index_xB = getBinIndex_xB(kaonp_xB);
                proton_spin = +1;
                ...
                //
                if(index_xB >= 0) {
                    vec_xB.push_back(kaonp_xB);
                    vec_kaonp_xB_1d[index_xB-1].push_back(kaonp_xB);
                    vec_kaonp_Q2_1d[index_xB-1].push_back(kaonp_Q2);    
                    vec_kaonp_phih_1d[index_xB-1].push_back(kaonp_Phi_h);
                    vec_helicity_1d[index_xB-1].push_back(helicity);
                    vec_kaonp_2phih_1d[index_xB-1].push_back(2*kaonp_Phi_h);
                    vec_kaonp_z_1d[index_xB-1].push_back(kaonp_z);
                    vec_kaonp_Pt_1d[index_xB-1].push_back(kaonp_PhT);
                    vec_kaonp_y_1d[index_xB-1].push_back(kaonp_y);
                    vec_kaonp_pol_1d[index_xB-1].push_back(kaonp_Pol);
                    vec_kaonp_eps_1d[index_xB-1].push_back(kaonp_epsilon);
                    vec_kaonp_spin_1d[index_xB-1].push_back(proton_spin);
                    vec_kaonp_sintheta_1d[index_xB-1].push_back(kaonp_sintheta);
                    vec_kaonp_phih_plus_1d[index_xB-1].push_back(kaonp_Phi_h);
                    vec_kaonp_phih_minus_1d[index_xB-1].push_back(kaonp_Phi_h);
                }
                //
                if(index_xQ2 >= 0) {
                ...
                target_abs_pol.Fill(std::abs(kaonp_Pol));
    ...
    std::vector<TH2D*> hists_kp = {
        ...
        &kp_Q2VsXb, &kp_Q2VsXf, &kp_Q2VsMom, &kp_Q2VsPhT, &kp_Q2VsZ, &kp_Q2VsY, &kp_Q2VsEta, &kp_Q2VsPhi_h, &kp_Q2vsSinTheta, &kp_Q2VsEps,
        &kp_PhTvsZ, &kp_PhTvsXb, &kp_PhTvsEta, &kp_PhTvsPhi_h, &kp_PhTvsSinTheta, &kp_PhTVsEps,
        &kp_zVsXb, &kp_zVsXf, &kp_zVsEta, &kp_zVsPhi_h, &kp_zvsSinTheta, & kp_zvsCosPhi, &kp_zVsEps,
        &kp_xBvsY, &kp_xBvsEta, &kp_xBvsSinTheta, &kp_xBvsCosPhi, &kp_xBVsEps,
    ...
    for (int x = 0; x < binxB; x++){ 
        ...
        ROOT::Math::Functor MLE([&](const double* p) {return AUL_loglike_withLL(p, vec_kaonp_phih_1d[x], vec_kaonp_spin_1d[x], vec_kaonp_eps_1d[x], vec_kaonp_pol_1d[x], vec_helicity_1d[x], vec_kaonp_sintheta_1d[x], true);}, 7);
        minimizer.SetFunction(MLE);
        minimizer.SetMaxFunctionCalls(50000);
        minimizer.SetMaxIterations(10000);
        minimizer.SetTolerance(0.001);
        minimizer.SetStrategy(1);
        minimizer.SetLimitedVariable(0, "Aul_sin", 0.00, 0.001, -0.5, 0.5); 
        minimizer.SetLimitedVariable(1, "Aul_2sin", 0.00, 0.000, -0.5, 0.5);
        minimizer.SetLimitedVariable(2, "All", 0.00, 0.00, -0.5, 0.5); 
        minimizer.SetLimitedVariable(3, "All_cos", 0.00, 0.00, -0.5, 0.5);
        minimizer.SetLimitedVariable(4, "Auu", 0.00, 0.000, -0.8, 0.8);
        minimizer.SetLimitedVariable(5, "Auu2", 0.00, 0.000, -0.8, 0.8);
        minimizer.SetLimitedVariable(6, "Alu", 0.00, 0.00, -0.5, 0.5);
    ...
    vector<double> Mat_AUL = {0.00817, 0.00492, 0.00281, 0.00143, 0.000199, -0.00102, -0.00269, -0.00165};
    vector<double> Mat_AUL_err = {0.000706, 0.000781, 0.000825, 0.000864, 0.000834, 0.000807, 0.000807, 0.000877, 0.00195};
    vector<double> Mat_xB = {0.114, 0.149, 0.173, 0.195, 0.219, 0.251, 0.296, 0.374, 0.502};
    TGraphErrors* graph2D_AUL_vs_xB = new TGraphErrors();
    TGraphErrors* graph2D_AUL_Matt = new TGraphErrors();
    //TGraphErrors* graph2D_corr_sin = new TGraphErrors();
    int p_idx1 = 0, p_idx1_1 = 0, p_idx1_2 = 0, p_idx1_3 = 0, p_idx1_4 = 0;
    for(int x = 0; x < binxB; x++){
        if (vec_kaonp_xB_1d[x].empty()) continue; // Skip empty bins
        ...
        for (double val : vec_kaonp_xB_1d[x]){
            sum_xB += val;
            if(val > max_x) max_x = val;
            if(val < min_x) min_x = val;
        }
        double mean_xB = sum_xB / vec_kaonp_xB_1d[x].size();
        cout << "bin " << x+1 << "  size: " << vec_kaonp_xB_1d[x].size() << "  xB_mean: " << mean_xB << "   delta err: " << (6.67*sqrt(1.2)*Mat_AUL_err[x]) / AUL_sin_err_2d[x] << endl;
        ...
        graph2D_AUL_vs_xB->SetPoint(p_idx1, mean_xB, 0.02);
        graph2D_AUL_vs_xB->SetPointError(p_idx1, 0.0, aul_sin_err/sqrt(1.2)); // No x error
        graph2D_AUL_Matt->SetPoint(p_idx1, Mat_xB[x], 0.04);
        graph2D_AUL_Matt->SetPointError(p_idx1, 0.0, 6.67*Mat_AUL_err[x]);
        //graph2D_corr_sin->SetPoint(p_idx1, mean_xB, Corr_sin_2d[x]);
        p_idx1++;
    }
    graph2D_AUL_vs_xB->SetMarkerStyle(20), graph2D_AUL_Matt->SetMarkerStyle(21);
    graph2D_AUL_vs_xB->SetLineColor(kAzure-5), graph2D_AUL_vs_xB->SetMarkerColor(kAzure-5);
    graph2D_AUL_Matt->SetLineColor(kRed-3), graph2D_AUL_Matt->SetMarkerColor(kRed-3);
    ...
    TMultiGraph *mg_Aut2D_xB = new TMultiGraph();
    mg_Aut2D_xB->SetTitle("A_{UL} vs x_{B} | #pi^{+}#pi^{-} lepton frame; x_{B}; A_{UL}");
    mg_Aut2D_xB->Add(graph2D_AUL_vs_xB);
    mg_Aut2D_xB->Add(graph2D_AUL_Matt);
    mg_Aut2D_xB->Draw("AP");
    mg_Aut2D_xB->GetYaxis()->SetRangeUser(0.0, 0.07);

    
    TLegend* legend = new TLegend(0.13, 0.75, 0.33, 0.88); // Adjust position (x1,y1,x2,y2)
    legend->AddEntry(graph2D_AUL_Matt, "Matthew RGC", "ep");
    legend->AddEntry(graph2D_AUL_vs_xB, "Lorenzo RGC", "ep");
    ...
    for (int i = 0; i < binxB; i++){
        double sum_xB = 0;
        for (double val_xB : vec_kaonp_xB_1d[i]) sum_xB += val_xB;
        double mean_xB = sum_xB / vec_kaonp_xB_1d[i].size();
        double frac = (1.0*vec_kaonp_xB_1d[i].size())/(1.0*vec_xB.size());
        x_points.push_back(mean_xB);
        y_points.push_back(frac);
        cout << "bin xB " << i+1 << ": " << frac << " entries (mean xB: " << mean_xB << ") --> AUL_err: " << AUL_sin_err_2d[i]/sqrt(1.2) <<  endl;
    }

    cout << "total entries " << vec_xB.size() << endl;
